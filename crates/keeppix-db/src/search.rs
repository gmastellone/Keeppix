...
impl<'a> SearchRepo<'a> {
    ...
    pub async fn run(
        ...
        let mut q = sqlx::query_as::<_, AssetRow>(&sql).bind(filter.bind());
        for b in &binds {
            q = bind_one(q, b);
        }
    ...
    pub async fn suggest(&self, ctx: &AuthContext, q: &str) -> Result<Vec<String>, DbError> {
        ...
        let sql = format!(
            "(SELECT e.camera_model AS s FROM asset_exif e \
              ...
              JOIN folders f ON f.id = a.folder_id \
              WHERE {} AND e.camera_model ILIKE $2 ESCAPE E'\\\\' \
              LIMIT 8) \
             UNION \
             (SELECT a.filename AS s FROM assets a \
              JOIN folders f ON f.id = a.folder_id \
              WHERE {} AND a.filename ILIKE $2 ESCAPE E'\\\\' \
              LIMIT 8) \
             LIMIT 10",
            filter.sql(),
            filter.sql()
